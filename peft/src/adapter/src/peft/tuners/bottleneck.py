...
class Linear(nn.Linear, AdapterLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.adapter_down.weight.data = self.adapter_down.weight.data.to(dtype=torch.bfloat16)
        self.adapter_up = nn.Linear(bottleneck_size, out_features, bias=False)
        self.adapter_up.weight.data = self.adapter_up.weight.data.to(dtype=torch.bfloat16)
    ...
    def forward(self, x: torch.Tensor):
        if self.disable_adapters:
            ...
            if self.adapter_type == "mh_adapter":
                ...
                result = F.linear(output, self.weight.to(torch.bfloat16), bias=self.bias)
            elif self.adapter_type == "output_adapter":
                # for output_adapter, x will pass the linear layer first and then the adapter
                x = F.linear(x, self.weight.to(torch.bfloat16), bias=self.bias)
                expected_dtype = torch.bfloat16
