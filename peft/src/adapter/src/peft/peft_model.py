...
class PeftModelForCausalLM(PeftModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            # concat prompt attention mask
            #print('打印2', attention_mask.shape)
            prefix_attention_mask = torch.ones(batch_size, self.peft_config.num_virtual_tokens).to(self.device)
            #attention_mask = torch.cat((prefix_attention_mask, attention_mask), dim=1)
        ...
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            past_key_values = self.get_prompt(batch_size)
            #print(input_ids.shape,'打印1',attention_mask.shape)
            kwargs.update(
            {
                "peft_attention_mask": prefix_attention_mask,
                "peft_past_key_values": past_key_values,
            }
        )
    ...
    def generate(self, **kwargs):
        ...
        #         shape_info = value.shape if value is not None else 'None'
        #     else: #
        #         shape_info = 'Not a Tensor'
    ...
    def prepare_inputs_for_generation(self, *args, **kwargs):
        ...
        if isinstance(self.peft_config, PromptLearningConfig):
            if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
                ...
                #你得搞清楚这个2 ,1 ，32,10,128 #batch size * token dim*num_layer*token数*attention heads是什么
     ...
     #   print(model_kwargs['peft_attention_mask'].shape,'打印mask shape')
        return model_kwargs #
